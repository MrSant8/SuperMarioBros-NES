...
TileMap::~TileMap()
{
	...
	if (surpriseBlock != nullptr)
	{
		surpriseBlock->Release();
		delete surpriseBlock;
		surpriseBlock = nullptr;
	}
}
...
AppStatus TileMap::Initialise()
{
	...
	surpriseBlock = new Sprite(img_tiles);
	if (surpriseBlock == nullptr)
	{
		LOG("Failed to allocate memory for laser sprite");
		return AppStatus::ERROR;
	}
	surpriseBlock->SetNumberAnimations(1);
	surpriseBlock->SetAnimationDelay(0, ANIM_DELAY);
	surpriseBlock->AddKeyFrame(0, dict_rect[(int)Tile::LASER_FRAME0]);
	surpriseBlock->AddKeyFrame(0, dict_rect[(int)Tile::LASER_FRAME1]);
...
void TileMap::Update()
{
	...
	for (auto& frag : fragments) {
		frag.x += frag.vx;
		frag.y += frag.vy;
		frag.vy += 0.3f; // Simulación de gravedad
		frag.lifetime--;
	}

	// Limpiar fragmentos que han desaparecido
...
bool TileMap::TestCollisionFromBelow(const AABB& box, int* py, Point* collisionTilePos)
{
	...
	if (IsTileSolid(tile) && tile != Tile::BLOCK_SQUARE1_TR && tile != Tile::BLOCK_SQUARE1_BR)
	{
		...
		if (tile == Tile::BLOCK_SQUARE1_TL && canBreak)
		{
			...
			for (int k = 0; k < 4; ++k) {
				BlockFragment frag;
				frag.x = pixel_x + (k % 2) * 8; // Small horizontal displacement
...
void TileMap::Render()
{
	...
	for (int i = 0; i < height; ++i)
	{
		for (int j = 0; j < width; ++j)
		{
			...
			if (dropBlock && i == laserTileY && j == laserTileX)
			{
				rc = dict_rect[(int)Tile::BLOCK_SQUARE1_BR];
			...
			if (IsTileSolid(tile) && tile != Tile::BLOCK_SQUARE1_TR && tile!= Tile::BLOCK_SQUARE2_TR && tile!=Tile::BLOCK_SQUARE2_BL)
			{
				...
				else if (tile == Tile::LASER && i != laserTileY && j != laserTileX)
				{
					...
					surpriseBlock->Draw((int)pos.x, (int)pos.y);
